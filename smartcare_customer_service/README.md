...
题目 8 不依赖 GPU，数据可以自建，适合 2 人小组在本地稳定演示；同时覆盖课程中的 RAG、Prompt Engineering、Function Calling、Agent 路由、Memory、工程实践和评估方法。相比单纯命令行实验，本项目提供 Web 界面、完整用户流程和 SQLite 历史记录，符合指南中的产品完整性加分项。
...
| 成员 | 主要职责 | 答辩讲解部分 |
| --- | --- | --- |
| 成员 A | 需求分析、知识库构建、RAG 检索、意图识别、评估实验 | 项目背景、技术原理、实验结果 |
| 成员 B | Web 界面、工具调用、SQLite 记忆、工程集成、README/演示 | 系统架构、功能演示、工程实现 |
